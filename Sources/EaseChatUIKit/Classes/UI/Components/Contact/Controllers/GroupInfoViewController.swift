...
@objc open class GroupInfoViewController: UIViewController {
    ...
    @objc required public convenience init(group: String,nameChanged: @escaping (String,String) -> Void) {
        self.init()
    ...
    private func fetchGroupInfo(groupId: String) {
        self.service.fetchGroupInfo(groupId: groupId) { [weak self] group, error in
            ...
            self.navigation.title = showName
    ...
    private func transfer() {
        let vc = ContactViewController(headerStyle: .transferOwner,provider: nil,ignoreIds: [self.chatGroup.owner])
        vc.confirmClosure = { [weak self] in
            vc.navigationController?.popViewController(animated: true)
            if let profile = $0.first {
                self?.transferConfirm(profile: profile)
            }
        }
    ...
    private func transferConfirm(profile: EaseProfileProtocol) {
        DialogManager.shared.showAlert(title: "", content: "转让群主给\(profile.nickName)?", showCancel: true, showConfirm: true) { [weak self] text in
            guard let `self` = self else { return }
...
extension GroupInfoViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    private func viewParticipants() {
        let vc = GroupParticipantsController(groupId: self.chatGroup.groupId)
