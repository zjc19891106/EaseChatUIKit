...
@objcMembers open class ContactSearchResultController: UITableViewController{
    ...
    @objc public required convenience init(headerStyle: ContactListHeaderStyle = .contact,action: @escaping (EaseProfileProtocol) -> Void) {
        self.init()
    ...
    public func updateSearchResults(for searchController: UISearchController) {
        ...
        if let searchText = searchController.searchBar.text?.lowercased() {
            self.searchResults = self.rawDatas.filter({ user in
                (user.nickName.lowercased() as NSString).range(of: searchText).location != NSNotFound && (user.nickName.lowercased() as NSString).range(of: searchText).length >= 0
            })
        }
        self.tableView.reloadData()
    }
