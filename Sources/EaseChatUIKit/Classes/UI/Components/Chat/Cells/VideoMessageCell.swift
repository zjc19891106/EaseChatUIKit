...
@objc open class VideoMessageCell: MessageCell {
    ...
    public private(set) lazy var content: ImageView = {
        ImageView(frame: .zero).tag(200).cornerRadius(Appearance.chat.imageMessageCorner)
    }()
    ...
    @objc required public override init(towards: BubbleTowards,reuseIdentifier: String) {
