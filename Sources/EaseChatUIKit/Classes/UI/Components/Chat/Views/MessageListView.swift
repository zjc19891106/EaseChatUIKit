...
extension MessageListView: UITableViewDelegate,UITableViewDataSource {
    ...
    private func registerMessageCell(tableView: UITableView,indexPath: IndexPath) -> MessageCell? {
        if let message = self.messages[safe: indexPath.row]?.message {
            ...
            default:
                ...
                if cell == nil {
                    cell = ComponentsRegister.shared.ChatMessageCell.init(towards: towards, reuseIdentifier: "EaseChatUIKit.ChatMessageCell")
                }
                return cell
            }
        } else {
    ...
    private func handleClick(area: MessageCellClickArea,entity: MessageEntity) {
        switch area {
        case .avatar:
            if ComponentViewsActionHooker.Chat.bubbleClicked != nil {
                if let user = entity.message.user {
                    ComponentViewsActionHooker.Chat.avatarClicked?(user)
                } else {
        ...
        case .reply:
            if ComponentViewsActionHooker.Chat.replyClicked != nil {
                ComponentViewsActionHooker.Chat.replyClicked?(entity)
            } else {
        ...
        case .bubble:
            if ComponentViewsActionHooker.Chat.bubbleClicked != nil {
                ComponentViewsActionHooker.Chat.bubbleClicked?(entity)
            } else {
                for handler in self.eventHandlers.allObjects {
    ...
    private func handleLongPressed(area: MessageCellClickArea,entity: MessageEntity) {
        if area == .bubble {
            if ComponentViewsActionHooker.Chat.bubbleLongPressed != nil {
                ComponentViewsActionHooker.Chat.bubbleLongPressed?(entity)
            } else {
                for handler in self.eventHandlers.allObjects {
            ...
            if ComponentViewsActionHooker.Chat.avatarLongPressed != nil {
                if let user = entity.message.user {
                    ComponentViewsActionHooker.Chat.avatarLongPressed?(user)
                } else {
                    let user = EaseProfile()
                    user.id = entity.message.from
                    ComponentViewsActionHooker.Chat.avatarLongPressed?(user)
...
extension MessageListView: IMessageListViewDriver {
    ...
    public func addMentionUserToField(user: EaseProfileProtocol) {
        ...
        let newString = NSAttributedString(string: "@\(user.nickName)", attributes: [.font: self.inputBar.inputField.font!, key: user, .foregroundColor: (Theme.style == .dark ? UIColor.theme.neutralColor98:UIColor.theme.neutralColor1)])
